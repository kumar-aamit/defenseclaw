...
#
# The Splunk HEC mock assertion is opt-in because the mock listener
# setup lives in the CI workflow, not here; the CI job sets
# ASSERT_SPLUNK=1 and passes SPLUNK_MOCK_LOG to this script.
#
# Usage:
...
# Optional: when set to a non-empty file path, the script asserts at
# least one line in that file has sourcetype `defenseclaw:judge` —
# used by the Splunk HEC mock scenario in CI.
SPLUNK_MOCK_LOG="${SPLUNK_MOCK_LOG:-}"
...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        ...
        --splunk-mock-log) SPLUNK_MOCK_LOG="$2"; shift 2;;
        --help|-h)
            sed -n '2,/^$/p' "$0"
            exit 0
            ;;
...
# tiny listener that appends each HEC request body to
# $SPLUNK_MOCK_LOG; we grep for the judge sourcetype which proves
# the ForwardJudgeResponse path actually fired end-to-end.
if [[ -n "$SPLUNK_MOCK_LOG" ]]; then
    if [[ ! -f "$SPLUNK_MOCK_LOG" ]]; then
        fail "splunk-mock-log not found at $SPLUNK_MOCK_LOG"
    fi
    if ! grep -q 'defenseclaw:judge' "$SPLUNK_MOCK_LOG"; then
